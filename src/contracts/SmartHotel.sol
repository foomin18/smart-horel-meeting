...
contract SmartHotel {
    ...
    mapping(uint16 => bool) private roomKeyStates;  //is room key opening
    //key is mod 86400
    ...
    event AppoCanseled(address indexed from, uint256 indexed timestamp, uint256 indexed span, uint8 index);
    ...
    function validateTimestamp(uint256 _t) private view {
        require(_t > deployTime, "before deploy");
        isMod86400(_t);
    }

    function isMod86400(uint256 _t) private pure {
        require(_t % 86400 == 0, "mod 86400");
    }
    ...
    //ユーザの予約確認
    function getUserAppo(address _addr) external view returns (UserBooking[] memory) {
    ...
    function buyTokens(uint256 _numTokens) external payable {
        ...
        (bool sent, ) = owner.call{value: hotelTokenPrice * _numTokens}("");
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < _numDays; ) {
            ...
            if (foundAppointment.isAppointment) monthlyAppointments[i] = 1;
            unchecked{ i++; }
        }
    ...
    {
        for (uint256 i = 0; i < _numDays; ) {
            require(scheduleByTimestamp[_roomId][_timestamp + (86400 * i)].isAppointment != true, 'Appo already exists');
            scheduleByTimestamp[_roomId][_timestamp + (86400 * i)] = Appointment(true, _name, msg.sender);
            unchecked{ i++; }
        }
    ...
    function removeUserBooking(uint8 _indexToDelete) internal {
        ...
        userBookings[msg.sender][_indexToDelete] = userBookings[msg.sender][userBookings[msg.sender].length - 1];
        userBookings[msg.sender].pop();
    }

    // 予約キャンセル
    function canselAppo(uint8 _index) external {
        ...
        for (uint256 i = 0; i < numDays; ) {
            ...
            require(scheduleByTimestamp[roomId][currDay].isAppointment == true, "not exist appo");
            require(scheduleByTimestamp[roomId][currDay].userAddress == msg.sender, "You haven't appo");
            //delete
        ...
        //UIに反映するためのイベントのemit
        emit AppoCanseled(msg.sender, timestamp, numDays, _index);
    }
    ...
    function checkIn(uint256 _timestamp, uint8 _index) external validTimestamp(_timestamp) { //チェックインする日にち予約番号
        ...
        //require(_timestamp == timeManager.marume86400(block.timestamp), "This is not today");
