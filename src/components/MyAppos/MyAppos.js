        ...
        {myAppos.map((appo, index) => (
          <div key={index} className="appointment">
            ...
            {appo.timestamp > timestamp ? (
              <button className='appobutton str ttu' onClick={() => canselAppo(index)}>Cancel Appointment</button>
            ) : !appo.isCheckedIn ? (
              <button className='appobutton str ttu' onClick={() => checkIn(Number(timestamp), index)}>Check In</button>
                ...
                <div>
                    <div className='str ttu'>Please press the checkout button when leaving.</div>
                    <button className='keybutton str ttu' onClick={() => canHandleKey(index)}>Room key operation</button>
